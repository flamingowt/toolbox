...
object readFromHbase {
  ...
  def main(args: Array[String]) {
    ...
    val news = sc.newAPIHadoopRDD(cfBadNews,classOf[TableInputFormat],classOf[ImmutableBytesWritable],classOf[Result])
      .map(x => x._2).map(x => {
      try{
        ...
        (tags.length,newsid+"\u0001"+title+"\u0001"+tags.mkString("_")+"\u0001"+content)
      }
catch {
  case e:Exception => (0,"")
}
    }).filter(_._1>0).filter(!_._2.equals("")).filter(_._2.split("\u0001").length>3).map(_._2)
     // .filter(x=>x.split("\u0001")(1).contains("发布")||x.split("\u0001")(1).contains("颁布")||x.split("\u0001")(1).contains("发行")||x.split("\u0001")(1).contains("宣传")||x.split("\u0001")(1).contains("宣布"))
    //  .filter(!_.split("\u0001")(1).contains("发布")).filter(!_.split("\u0001")(1).contains("颁布")).filter(!_.split("\u0001")(1).contains("发行")).filter(!_.split("\u0001")(1).contains("宣传")).filter(!_.split("\u0001")(1).contains("宣布")).takeSample(false,100000,10)
     .saveAsTextFile(out)
